...
__global__ void gpu_gaussian(int width, int height, float *image, float *image_out)
{
    ...
    int i = threadIdx.y * BLOCK_SIZE_SH + threadIdx.x;

    __shared__ float sh_block[BLOCK_SIZE_SH * BLOCK_SIZE_SH];

    if (index_x < width && index_y < height)
        sh_block[i] = image[offset_t];
        
    if (index_x > (width - 2) || index_y > (height - 2)) return;
    
    if (threadIdx.x < (BLOCK_SIZE_SH - BLOCK_SIZE) && (index_x + BLOCK_SIZE) < width)
        sh_block[i + BLOCK_SIZE] = image[offset_t + BLOCK_SIZE];
    if (threadIdx.y < (BLOCK_SIZE_SH - BLOCK_SIZE) && (index_y + BLOCK_SIZE) < height)
        sh_block[i + BLOCK_SIZE * BLOCK_SIZE_SH] = image[offset_t + BLOCK_SIZE * width];

    if (threadIdx.x == 0 && threadIdx.y == 0 && (index_x + BLOCK_SIZE_SH) < width && (index_y + BLOCK_SIZE_SH) < height)
        for (int x = BLOCK_SIZE; x < BLOCK_SIZE_SH; x++)
            for (int y = BLOCK_SIZE; y < BLOCK_SIZE_SH; y++)
                sh_block[y * BLOCK_SIZE_SH + x] = image[offset_t + y * width + x];
    __syncthreads();

    int offset   = (index_y + 1) * width + (index_x + 1);
        
    image_out[offset] = gpu_applyFilter(&sh_block[i],
...
__global__ void gpu_sobel(int width, int height, float *image, float *image_out)
{
    ...
    float gx = gpu_applyFilter(&sh_block[i], BLOCK_SIZE_SH, sobel_x, 3);
    float gy = gpu_applyFilter(&sh_block[i], BLOCK_SIZE_SH, sobel_y, 3);
...
int main(int argc, char **argv)
{
    ...
    // Step 2: Apply a 3x3 Gaussian filter
    {
        ...
        // Launch the GPU version
        gettimeofday(&t[0], NULL);
        cudaMemcpy(d_image_out[1], d_image_out[0], image_size * sizeof(float), cudaMemcpyDeviceToDevice);
